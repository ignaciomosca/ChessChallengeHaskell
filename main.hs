...
findCandidate Piece { row = r, col = c, piece = p } Board { m = m, n = n, usedPieces = up }
    = Data.Set.singleton
        [ bs
        | rr <- [1 .. m]
        , cc <- [1 .. n]
        , let piece = createPiece (p, rr, cc)
        , let bs = place Board {m = m, n = n, usedPieces = up} piece
        , isSafe bs piece
        ]

solution :: Board -> [ChessPiece] -> Set Board -> Set Board
solution board (p : ps) solutions =
    solution board ps (solutions `union` findCandidate p board)
